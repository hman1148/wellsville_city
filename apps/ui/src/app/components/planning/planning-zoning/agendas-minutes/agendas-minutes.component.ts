...
export class AgendasMinutesComponent implements OnInit {
  readonly meetingsStore = inject(MeetingsStore);

  ngOnInit(): void {
    this.meetingsStore.resolveMeetings('planning-zoning');
  }
  ...
  pastMeetings = computed(() => {
    return this.meetingsStore.meetings().filter(m => m.status !== 'upcoming').slice(0, 5);
  });
