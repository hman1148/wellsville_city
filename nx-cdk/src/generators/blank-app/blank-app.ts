...
import { discoverExportsFromDependencies } from './utils/ast-parser';

export async function blankAppGenerator(tree: Tree, options: BlankAppGeneratorSchema) {
  ...
  // Update the tsconfig.json to set noEmit: true
  const tsconfigPath = joinPathFragments(projectRoot, 'tsconfig.json');
  if (tree.exists(tsconfigPath)) {
    const tsconfig = JSON.parse(tree.read(tsconfigPath, 'utf-8') as string);
...
export default blankAppGenerator;
